...
const Converter: FC<ConverterProps> = () => {
  ...
  const [currentCurrenciesFirst, setCurrentCurrenciesFirst] = useState<TCurrency>(currencies[0])
  const [currentCurrenciesSecond, setCurrentCurrenciesSecond] = useState<TCurrency>(currencies[0])
  ...
  useEffect(() =>{ 
    ...
    let value = (currentValue * currentCurrenciesFirst.priceUsd) / currentCurrenciesSecond.priceUsd;
  ...
  return (
    ...
    <div className={styles.converter}>
      <div className={styles.currencies}>
        ...
        <input className={styles.current_value} onChange={(e) => handleCurrentValue(Number(e.target.value))} type="number" />
      <input readOnly value={currentCurrenciesFirst?.name} type="text" />
      <button onClick={() => setOpenListFirst(!openListFirst)}>Список валют</button>
      {openListFirst ? 
      <div className={styles.currencies_list}>
        { currencies.map((el) =>(
          <p onClick={() => handleFirstList(el)}>{el.name + ' ' + el.priceUsd + '$'}</p>
        ))} 
      </div>: '' } 
      </div>
      ...
      <button onClick={() => setOpenListSecond(!openListSecond)}>Список валют</button>
      {openListSecond ? 
      <div className={styles.currencies_list}>
        { currencies.map((el) =>(
          <p onClick={() => handleSecondList(el)}>{el.name + ' ' + el.priceUsd + '$'}</p>
        ))} 
      </div>: '' } 
      </div>
      <div className={styles.totalValue}>
        <input readOnly value={currentCurrenciesFirst.name && currentCurrenciesSecond.name ?
        currentValue + ' ' + currentCurrenciesFirst.name + ' = ' + totalValue + ' ' + currentCurrenciesSecond.name
          : ''}
         type="text" />
      </div>
    ...
    <div className={styles.currencies_wrapper}>
      <div className={styles.currencies_info}>
        ...
        <div>
          <p>Название: {currentCurrenciesFirst.name + ' ' + currentCurrenciesFirst.symbol}</p>
          <p>Цена в $: {currentCurrenciesFirst.priceUsd}</p>
          <p>Изменения за 24 часа: {currentCurrenciesFirst.changePercent24Hr} %</p>
        </div>
        : ''}
      </div>
      <div className={styles.currencies_info}>
        ...
        <div>
          <p>Название: {currentCurrenciesSecond.name + ' ' + currentCurrenciesFirst.symbol}</p>
          <p>Цена в $: {currentCurrenciesSecond.priceUsd}</p>
          <p>Изменения за 24 часа: {currentCurrenciesSecond.changePercent24Hr} %</p>
        </div>
        : ''}
      </div>
      ...
      <div className={styles.total_value_info}>
        ...
        <p>{` или ${currentValue * currentCurrenciesFirst.priceUsd}$ долларов`}</p>
      </div> : ''}
    </div>
    </>
    );
